...
public class RunController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<IEnumerable<RunActivity>>> GetAllRuns()
    {
        var runs = await _runService.GetAllRunsAsync();
        if (runs is null) return new List<RunActivity>();
        return Ok(runs);
    }

    [HttpGet("date/{date}")]
    public async Task<ActionResult<IEnumerable<RunActivity>>> GetRunsByDate(DateTime date)
    {
    ...
    [HttpGet("{id}")]
    public async Task<ActionResult<RunActivity>> GetRunById(int id)
    {
    ...
    [HttpPost]
    public async Task<ActionResult<RunActivity>> AddRun(RunDto runDto)
    {
    ...
    [HttpPut("{id}")]
    public async Task<ActionResult<RunActivity>> UpdateRunAsync(int id, RunDto runDto)
    {
        try
        {
