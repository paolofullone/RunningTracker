...
namespace UnitTests
{
    public class RunControllerTests
    {
        ...
        {
            // Arrange
            var runs = new List<Run> { new Run { Id = 1, Distance = 5.0 } };
            ...
            var returnedRuns = Assert.IsAssignableFrom<IEnumerable<Run>>(okResult.Value);
        ...
        public async Task GetRunsByDate_ReturnsOkWithRuns_WhenServiceReturnsRuns()
        {
            ...
            var runs = new List<Run> { new Run { Id = 1, Date = date } };
        ...
        {
            // Arrange
            var run = new Run { Id = 1, Distance = 10.0 };
            ...
            // Assert
            var okResult = Assert.IsType<OkObjectResult>(result.Result);
            var returnedRun = Assert.IsType<Run>(okResult.Value);
        ...
        {
            // Arrange
            _mockRunService.Setup(s => s.GetRunByIdAsync(1)).ReturnsAsync((Run)null);
        ...
        public async Task AddRun_ReturnsCreatedAtAction_WithNewRun()
        {
            ...
            var run = new Run { Id = 1, Distance = 5.0 };
            ...
            var returnedRun = Assert.IsType<Run>(createdResult.Value);
            Assert.Equal(run, returnedRun);
        }
        ...
        public async Task UpdateRun_ReturnsOkWithUpdatedRun_WhenRunExists()
        {
            ...
            var updatedRun = new Run { Id = 1, Distance = 7.0 };
