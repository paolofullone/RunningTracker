...
namespace UnitTests
{
    public class RunServiceTests
    {
        ...
        public async Task AddRunAsync_Should_ReturnCreatedRun()
        {
            ...
            _mockRunRepository.Setup(r => r.AddRunAsync(It.IsAny<Run>())).ReturnsAsync(expectedRun);
            ...
            _mockRunRepository.Verify(r => r.AddRunAsync(It.IsAny<Run>()), Times.Once);
        }
        [Trait("UnitTests", "Service")]
        [Fact]
        ...
        {
            // Arrange
            var expectedRuns = new List<Run> { new Run(), new Run() };
        ...
        public async Task GetRunsByDateAsync_Should_ReturnRunsForDate()
        {
            ...
            var expectedRuns = new List<Run> { new Run { Date = testDate } };
        ...
        public async Task GetRunByIdAsync_Should_ReturnRun_WhenRunExists()
        {
            // Arrange
            var runId = 1;
            var expectedRun = new Run { Id = runId };
        ...
        public async Task UpdateRunAsync_Should_ReturnUpdatedRun_WhenRunExists()
        {
            ...
            var runId = 1;
            var runDto = new RunDto();
            var existingRun = new Run { Id = runId };
            var updatedRun = new Run { Id = runId };
        ...
        public async Task UpdateRunAsync_Should_ThrowKeyNotFoundException_WhenRunDoesNotExist()
        {
            ...
            var runId = 1;
            var runDto = new RunDto();
            _mockRunRepository.Setup(r => r.GetRunByIdAsync(runId)).ReturnsAsync((Run)null);

            // Act & Assert
            await Assert.ThrowsAsync<KeyNotFoundException>(() => _runService.UpdateRunAsync(runId, runDto));
            _mockRunRepository.Verify(r => r.GetRunByIdAsync(runId), Times.Once);
            _mockRunRepository.Verify(r => r.UpdateRunAsync(It.IsAny<Run>()), Times.Never);
        }
